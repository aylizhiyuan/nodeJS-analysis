...
//默认情况下是不会打印的，只有你使用set DEBUG=static:app才可以被打印出来。 
//Mac下是export DEBUG=static:app
//这么做的好处是程序员切换到debug模式的时候开启调试模式。。。。
...
class Server {
    ...
    async request(req, res) {
        ...
        try {
            //使用fs.state来判断文件的类型，它本身是个回调函数
            //所以，一般的做法是封装到promise里面，但这里有个非常简单的办法
            //promisify就是一个将结果放入Promise里面的方法
            let statObj = await stat(filepath);
            if (statObj.isDirectory()) {//如果是目录的话，应该显示目录 下面的文件列表
                //这里用到了模板引擎，直接返回模板引擎渲染后的html文件
